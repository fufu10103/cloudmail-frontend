...
export function renderEmailContent(content, apiBase) {
  ...
  // 深色模式下强制邮件内容适配深色（注入CSS覆盖邮件自带的白色背景）
  const darkCss = `
    <style>
      .mail-content { color-scheme: light; }
      html.dark .mail-content {
        color-scheme: dark;
      }
      html.dark .mail-content body,
      html.dark .mail-content div,
      html.dark .mail-content td,
      html.dark .mail-content p {
        background-color: transparent !important;
        color: #e5e7eb !important;
      }
      html.dark .mail-content a {
        color: #93c5fd !important;
      }
      html.dark .mail-content img {
        filter: brightness(0.9);
      }
    </style>`;

  return darkCss + '<div class="mail-content">' + html + '</div>';
}
